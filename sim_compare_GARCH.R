...
garch_estimate = function (x) {
  model = garchFit(~garch(1,1), x, trace = F, include.mean = F)
  res = c(unname(coef(model)))
...
truth = as.matrix(read.csv("./sim_data/GARCH/GARCH_original.csv", header = F))
x_obs = as.matrix(read.csv("./sim_data/GARCH/GARCH_miss1.csv", header = F))
benchmarks = as.matrix(read.csv("./sim_data/GARCH/benchmarks_miss1.csv"))
WI = as.matrix(read.csv("./sim_data/GARCH/GARCH_WI_miss1.csv", header = F))
WI_Kalman = as.matrix(read.csv("./sim_data/GARCH/GARCH_WI_Kalman_miss1.csv", header = F))
...
for (sim in 1:1000) {
  ...
  if (sim == 1) {
    model_coef[[1]] = garch_estimate(dta)
    model_coef[[2]] = garch_estimate(lin)
    model_coef[[3]] = garch_estimate(spl)
    model_coef[[4]] = garch_estimate(Kalman)
    model_coef[[5]] = garch_estimate(PT)
    model_coef[[6]] = garch_estimate(wass)
    model_coef[[7]] = garch_estimate(kwass)
    model_coef[[8]] = garch_estimate(wass_Kalman)
    model_coef[[9]] = garch_estimate(kwass_Kalman)
  } else {
    model_coef[[1]] = rbind(model_coef[[1]], garch_estimate(dta))
    model_coef[[2]] = rbind(model_coef[[2]], garch_estimate(lin))
    model_coef[[3]] = rbind(model_coef[[3]], garch_estimate(spl))
    model_coef[[4]] = rbind(model_coef[[4]], garch_estimate(Kalman))
    model_coef[[5]] = rbind(model_coef[[5]], garch_estimate(PT))
    model_coef[[6]] = rbind(model_coef[[6]], garch_estimate(wass))
    model_coef[[7]] = rbind(model_coef[[7]], garch_estimate(kwass))
    model_coef[[8]] = rbind(model_coef[[8]], garch_estimate(wass_Kalman))
    model_coef[[9]] = rbind(model_coef[[9]], garch_estimate(kwass_Kalman))
  }
  ...
  for (i in 1:9) {
    if (i == 1) {
      hist(acfs[,j], prob = TRUE, col = "lightblue", 
           main = paste0("Ground truth: ACF Lag", j - 1),
           xlab = "")
      ...
      hist(acfs[,((i - 1) * 6) + j], prob = TRUE, col = "lightblue", 
           main = paste0(method, ": ACF Lag ", j - 1),
           xlab = "")
      lines(density(acfs[,j]), col = "blue", lwd = 2)
    }
  }
...
truth = as.matrix(read.csv("./sim_data/GARCH/GARCH_original.csv", header = F))
x_obs = as.matrix(read.csv("./sim_data/GARCH/GARCH_miss2.csv", header = F))
benchmarks = as.matrix(read.csv("./sim_data/GARCH/benchmarks_miss2.csv"))
WI = as.matrix(read.csv("./sim_data/GARCH/GARCH_WI_miss2.csv", header = F))
WI_Kalman = as.matrix(read.csv("./sim_data/GARCH/GARCH_WI_Kalman_miss2.csv", header = F))
